...
class AssetService(
    ...
    fun getPricingInfo(denoms: List<String>): MutableMap<String, BigDecimal> {
        ...
        val res = khttp.get(
            url = "${props.pricingUrl}/api/v1/pricing/marker/denom/list",
            params = mapOf("denom[]" to denoms.joinToString(","))
        )

        return if (res.statusCode == 200) {
            try {
                res.jsonArray.mapNotNull { ele ->
        ...
        } else mutableMapOf<String, BigDecimal>().also { logger.error("Error reaching Pricing Engine: ${res.jsonObject}") }
    }

    fun getPricingForNhash(): Map<String, BigDecimal> {
        val url =
            "https://www.dlob.io/aggregator/external/api/v1/order-books/pb18vd8fpwxzck93qlwghaj6arh4p7c5n894vnu5g/daily-price"
        val res = khttp.get(url = url)

        return if (res.statusCode == 200) {
            try {
                res.jsonObject.let { mapOf("nhash" to it.getString("latestDisplayPricePerDisplayUnit").toBigDecimal()) }
            } catch (e: Exception) {
                mapOf<String, BigDecimal>().also { logger.error("Error: $e") }
            }
        } else mapOf<String, BigDecimal>().also { logger.error("Error reaching Pricing Engine: ${res.jsonObject}") }
    }
